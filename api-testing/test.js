...
var fs = require('fs');
nba.leaguePlayerGeneralStats({LeagueID: "00", Season: "2018-19", MeasureType: "Defense"}).then(function(data){
  for (let i = 0; i < 530; i++) {
  	var name = data["LeagueDashPlayerStats"][i.toString()]["PLAYER_NAME"];
  	var ID = data["LeagueDashPlayerStats"][i.toString()]["PLAYER_ID"];
